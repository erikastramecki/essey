...
- **Hook build/gate state (VERIFIED 2026-08-30; SUPERSEDED 2026-09-02):** ~~the reworked hook + its three
  ...
  doc.** Current: the contracts are **COMMITTED** (`ae143bc`) and the gate verdict is **WITHDRAWN** (counter ZERO), grounded in
...
| A-2 | not-new | Already known + already tested (deploy precondition #2). ⚠️ **THIS PRECONDITION NO LONGER HOLDS — re-derive before any launch deploy.** The entire supply was redeemed 2026-09-09T23:14:59Z (tx `0x4df5445e…f4e4`); `totalSupply()` is now **0** and the reserve retains the equity basket. Deliberate pre-launch unwind, not a fault — but `EsseyReserveHook.sol:308` depends on this and the consequence here is escalated to "seed and launch lost", so it must be re-stated against the relaunched token rather than carried. Historical note follows: re-verified on chain 2026-09-02 that the precondition then held: ESSEY `totalSupply == balanceOf(0x93e6…4B9E) == 8,888,888,888e18`. A-3 escalated its consequence from "surcharge lost" to "seed and launch lost." | `EsseyReserveHook.sol:308` |
