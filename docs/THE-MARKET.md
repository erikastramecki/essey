...
**Get a Don → raise its Tier → the Bell rings → stock lands in your Vault.** Fees from real
activity — trading, royalties, loan interest — pool up, and the Bell splits the pot across every active
Don, by Tier, paid in real stock. No emissions anywhere in the loop: everything distributed was first
earned.

## The mechanics

### Don — membership as an asset
...
### Vault — the Don IS a wallet
Every Don deploys its own token-bound wallet (ERC-6551) at mint, at a deterministic address only the
collection can create. Payouts land in the Vault, not your EOA — so selling a Don sells a *portfolio*,
and everything it ever earned travels with it. One honest consequence: sellers should claim and drain
before selling; a Vault's contents belong to the Don, not to you.

### Tier — staking that buys weight, not yield
Stake $ESSEY to raise a Don's Tier; higher Tier means a larger slice of every Payout. Half of every
activation fee is burned. Tier clears when the Don changes hands — deliberately: status is a recurring
...
accumulator math (a global per-weight index; each Don tracks its debt against it), so a ring costs the
same gas whether ten Seats are active or two thousand.

### Payout — a fee-share, never a dividend
The pot splits pro-rata by Tier weight into every active Don's Vault, paid in the fee stable or —
per-Don, opt-in — split across up to three chosen stocks at the claim edge. Conversion is oracle-checked and
...
A two-sided vault AMM holding Don inventory and an $ESSEY reserve, priced at the live floor: **buy** the next
Don, **snipe** an exact number for a premium, or **sell** one back to the float. Every trade's fee
...
proven end-to-end on-chain (a Bell claim delivering AAPL + NVDA into a Don's Vault; a roll settling and
...
the Tape's full indexer, and on-chain Don art. **Not yet on mainnet:** everything here is testnet play
