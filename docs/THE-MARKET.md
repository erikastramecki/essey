...
### Cases — fair-value packs
Buy a Case in $ESSEY; a committed on-chain draw delivers a real stock unit straight to your wallet.
**Every unit is ~the case's value — the draw only ever decides which name.** Rarity is scarcity of the
name, never payout size; there are no multipliers, by design and by hard gate (a variant with uneven
prizes would require a hardened VRF and jurisdiction review before it could exist). The bankroll is
provably solvent: buying reverts unless a real, already-deposited prize unit backs your unopened case.
Sell any unit back at oracle value minus a floored spread; both fee legs feed the Bell.
...
## What's enforced today vs designed

Built and adversarially audited (published rounds, clean or not): Seats, Vaults, Tiers, the Bell, the
claim-edge converter, Notes, $ESSEY, the Exchange, the mint distributor, Cases. Designed, not yet built:
royalty routing and the loan-interest→Bell share (the After Hours engines), the Tape's indexer, and
on-chain Seat art. Nothing is deployed yet; until it is, this page says "designed to," not "does."
