...
## On-chain verifier — DONE

The last mile: the proof actually verified on a live chain. `vk.ExportSolidity` generates a standard
Groth16 verifier contract; `onchain-verify.sh` exports it, deploys it to a local EVM (anvil), and
verifies a real proof end to end:

- **Verified on-chain** — a real Groth16 proof passes `verifyProof(uint256[8], uint256[1])` (view
  reverts on an invalid proof; ours does not).
- **~217k gas** — the flat cost of the on-chain check. It is **independent of circuit size**: the
  same ~217k whether the circuit behind the proof is 2k or 2.4M constraints, because Groth16 verify
  is a fixed pairing + one MSM term per public input. Cheaper than a Uniswap swap.
- **Sound** — a bogus public input is rejected on-chain.

This is the number that matters for "any chain": deploy this ~one-time verifier contract on any EVM
chain and every portable proof verifies for a few hundred k gas, no oracle required. (The
portable-proof verifier is generated by the identical export, with one public input per loan plus
the lookup-argument commitments; same shape, same order-of-magnitude cost.)

## One-command demo — DONE

`demo.sh` runs the entire pipeline on a LIVE Pyth price and narrates it — the thing you run in front
of someone:

```
bash circuit/poseidon/demo.sh
```

  [0] fetch a live BTC price from Pyth (Hermes)
  [1] verify it's signed by 13 real Pyth guardians + Merkle-included   (native)
  [2] load proving keys                                                (cached; first run does the ~6min setup)
  [3] prove a solvent loan against that price in zero knowledge        (~13s, 196-byte proof)
  [4] verify the proof off-chain
  [5] export the Solidity verifier + exact calldata
  [6] deploy to a fresh EVM and VERIFY ON-CHAIN                        (~338k gas)

Measured on-chain gas: **~338k** for the full portable proof (the extra ~120k over a plain proof is
the one Pedersen commitment check the lookup arguments introduce; a commitment-free proof is ~217k —
see `onchain-verify.sh`). Both are flat, circuit-size-independent, and cheaper than many DeFi actions.
After the one-time setup, the whole demo runs in ~1 minute and every step is real.

One integration detail worth recording: `MarshalSolidity()` encodes the proof commitments as a
length-prefixed slice, so the raw bytes are not 32-byte aligned; the prover strips the 4-byte prefix
to build clean `verifyProof(uint256[8],uint256[2],uint256[2],uint256[N])` calldata, and proving uses
`solidity.WithProverTargetSolidityVerifier` so the proof matches the exported verifier.
