...
const band = (v: bigint): number => {
  ...
  return Math.max(1, Math.min(8, Math.floor(Math.log10(Math.max(1, scrip)) * 2)));
};

// ---------------------------------------------------------------------------------------------
// 1) STAKEOUT — the target list (away Dons, own Dons excluded)
// ---------------------------------------------------------------------------------------------
export function StakeoutFile({ open, under, onClose }: { open: boolean; under: boolean; onClose: () => void }) {
  const g = useGame();
  const navigate = useNavigate();
  const { targets } = useAwayTargets((g.dons ?? []).map((d) => d.id));
  return (
    <CaseFile open={open} under={under} jacket="carbon" tab="STAKEOUT · TARGETS ON THE TAPE" head="SURVEILLANCE · AWAY DONS" onClose={onClose}>
      <p className="g-typed" style={{ marginBottom: 10 }}>
        every depart prints on the Tape: somewhere, work is getting done — and back there, a House is standing open.
        exposure shows in <b>bands, not numbers</b>. garrisons stay redacted; the commit was frozen at their depart.
      </p>
      {!g.configured ? (
        <div className="g-typed"><p><span className="lbl">THE WORD</span>the stakeout ledger opens with the season. contracts pending — nothing to case yet, and casing nothing is free.</p></div>
      ) : targets === null ? (
...
// ---------------------------------------------------------------------------------------------
type RaidPhase = "order" | "committing" | "committed" | "revealing" | "report" | "error";
...
export function HitOrderFile({ targetDonId, open, onClose }: { targetDonId: bigint | null; open: boolean; onClose: () => void }) {
  const g = useGame();
  ...
  const windowOpen = secret !== null && now >= revealOpensAt && now <= revealClosesAt;
  const windowMissed = secret !== null && now > revealClosesAt;

  const reveal = async () => {
    if (!g.address || !secret || !secret.raidId || phase !== "committed" || !windowOpen) return;
    setErr(null); setPhase("revealing");
    try {
      let fee = 0n;
      ...
      const rcpt = await pub.getTransactionReceipt({ hash });
      const settledEvt = raidEngineAbi.find((x) => x.type === "event" && x.name === "RaidSettled");
      for (let i = 0; i < 12; i++) {
        await new Promise((r) => setTimeout(r, 5000));
        try {
          const logs = await pub.getLogs({
            address: GAME_ADDR.raidEngine, event: settledEvt as never,
            args: { raidId } as never, fromBlock: rcpt.blockNumber, toBlock: "latest",
          });
          if (logs.length > 0) {
            setOutcome(Number((logs[0] as { args?: { outcome?: number } }).args?.outcome ?? 0));
            g.refresh();
            break;
          }
        } catch { /* keep polling */ }
      ...
      setPhase("committed"); setErr(niceError(e));
    }
  };
    ...
    <CaseFile open={open} stack2 jacket="carbon" tab={`HIT ORDER · TARGET №${targetDonId.toString()}`} head="HIT ORDER" onClose={onClose}>
            ...
            <div className="g-photowin">
              <div className="g-pframe">
                <div className="g-surv" style={{ border: "none" }}>
                  ...
                  <DonImg id={targetDonId} alt={`target — Don №${targetDonId.toString()}`} />
                </div>
              </div>
              <div className="g-pcap">target · don №{targetDonId.toString()}</div>
            </div>
            <div className="g-typedfields">
              <p className="g-serifname">House Robbery</p>
              <div className="g-tf"><label>Target</label><b>DON №{targetDonId.toString()} — away, House standing open</b></div>
              <div className="g-tf"><label>Mode</label><b>HOUSE ROBBERY · commit now, reveal in 10–40 min</b></div>
              <div className="g-tf"><label>Odds</label><b>vs UNKNOWN garrison — 5–70%, crew vs the door. a range, because fog.</b></div>
              <div className="g-tf"><label>Stakes</label><b>◫ {fmtAmt(COMMIT_FEE)} order fee, sunk · a miss can hospitalize the crew 48h</b></div>
            </div>
          </div>

          <hr className="g-dashrule" />
          <p className="g-pcap-h">THE CREW — who goes through the door · five max</p>
          {available.length === 0 ? (
            <p className="g-typed" style={{ fontSize: 11 }}>
              no Hitters out of the hospital. a raid needs muscle — retain a crew at the RAP SHEETS.
            </p>
          ) : (
            <div className="g-provs">
              {available.map((h) => {
                const cooling = h.lastAttemptAt > 0 && now - h.lastAttemptAt < 20 * 3600 * 1000;
                return (
                  <button key={h.id.toString()} aria-pressed={picked.has(h.id.toString())}
                    title={cooling ? "on cooldown — hunts at diminished odds, (t/20h)²" : undefined}
                    onClick={() => setPicked((p) => {
                      const n = new Set(p);
                      const k = h.id.toString();
                      if (n.has(k)) n.delete(k); else if (n.size < MAX_CREW) n.add(k);
                      return n;
                    })}>
                    ♟ №{h.id.toString()}{cooling ? " ·⏳" : ""}
                  </button>
                );
              })}
            </div>
          )}
          <p className="g-provnote">the order seals the crew list. nobody — including you — can change it after the wax takes the die. ⏳ = cooling: allowed, at (t/20h)² odds.</p>
          {err && <div className="g-err">{err}</div>}
          <WaxSeal disabled={!!gate || phase === "committing"} hint={sealHint} onCommit={commit} />
        </>
      )}
      ...
      {(phase === "committed" || phase === "revealing") && secret && (
        <div className="g-envelope waiting">
          <BigEnvelope />
          <p className="g-typed" style={{ textAlign: "center", maxWidth: "38ch" }}>
            <span className="lbl">THE ENVELOPE IS SEALED</span>
            committed {fmtHMS((now - secret.committedAt) / 1000)} ago. the reveal window is 10–40 minutes
            after the commit — it kills mempool sniping in both directions.
          </p>
          {windowMissed && (
            <p className="g-err">the window closed — the order lapses and the fee burns. anyone may file the forfeit.</p>
          )}
          {err && <div className="g-err">{err}</div>}
          <button className="g-stampbtn" disabled={phase === "revealing" || !windowOpen || !secret.raidId} onClick={reveal}>
            {phase === "revealing" ? "OPENING…"
              : windowMissed ? "WINDOW MISSED"
              : !windowOpen ? `OPENS IN ${fmtHMS((revealOpensAt - now) / 1000)}`
      ...
      {phase === "report" && (
        <div>
          ...
          <div className="g-rapsheet">
            <div className="g-canon" style={{ display: "block", paddingRight: 90 }}>
              {outcome !== null && OUTCOME_STAMP[outcome] ? (
                <Stamp corner big thump ok={OUTCOME_STAMP[outcome].ok} style={{ top: -6 }}>{OUTCOME_STAMP[outcome].label}</Stamp>
              ) : (
                <Stamp corner carbon style={{ top: -6 }}>PENDING</Stamp>
              )}
            ...
            <div className="g-typed" style={{ marginTop: 8 }}>
              {outcome !== null && OUTCOME_STAMP[outcome] ? (
                OUTCOME_STAMP[outcome].ok
                  ? <p><span className="lbl">THE TAKE</span>the door gave. hit tax burned at the cage; the rest landed in your Don's vault — already banked, already yours.</p>
                  : <p><span className="lbl">THE DOOR HELD</span>the House was waiting. check the hospital ward — returning fire is real, and the defended punish hard.</p>
              ) : (
                <p><span className="lbl">REPORT PENDING</span>the entropy roll is still in the wire — the stamp lands when the keeper's callback settles the outcome. the garrison may also need its own reveal first.</p>
              )}
              <p><span className="lbl">CREW</span>cooldown starts now. hunting early is allowed — at sharply reduced odds. that's strategy, not a gate.</p>
            </div>
          </div>
          <div style={{ marginTop: 12, textAlign: "center" }}>
            <button className="g-stampbtn ok" onClick={onClose}>FILE THE REPORT · CLOSE</button>
          </div>
        </div>
      )}
    </CaseFile>
