...
export function HouseFile({
  ...
  const [busy, setBusy] = useState<
    null | "deploy" | "bank" | "repair" | "upgrade"
  >(null);
  ...
  const garrisonIds =
    don && g.address
      ...
      : [];
  const moveGate = !g.configured
    ? "the escrow opens with the season — contracts pending"
    : !g.connected
      ? "connect a wallet to move value"
      : don?.away
        ? "your Don is in the field — the escrow moves only while home"
        : null;

  return (
    <CaseFile
      open={open}
      tab={`PROPERTY FILE · DEED №${don && don.deedId !== 0n ? don.deedId.toString().padStart(4, "0") : "————"}`}
      head="PROPERTY FILE"
      onClose={onClose}
    >
      {!don ? (
        ...
        <>
          <div className="g-canon">
            <Stamp corner ok={damage === 0} style={{ fontSize: 11 }}>
              {damage === 0 ? "INTACT" : "INCIDENT"}
            </Stamp>
            <div className="g-photowin">
              ...
              <div className="g-pframe">
                <ExhibitStill
                  square
                  scene={HOUSE_TIER_SCENES[Math.min(tier ?? 0, 4)]}
                  damaged={damage > 0}
                />
              </div>
            ...
            <div className="g-typedfields">
              ...
              <div className="g-tf">
                ...
                <b>
                  {tier !== null
                    ? `${tier + 1} of 5 — "${HOUSE_TIER_LINES[Math.min(tier, 4)]}"`
                    : "— claims itself at first dispatch"}
                </b>
              </div>
              <div className="g-tf">
                <label>Deed</label>
                <b>
                  {don.deedId !== 0n
                    ? "held in own Vault · soulbound this season"
                    : "not yet claimed — the first depart mints it, free"}
                </b>
              </div>
              <div className="g-tf">
                ...
                <b>
                  {damage === 0
                    ? "100% — no damage on file"
                    : `${condPct}% 🔧 — earning rate is cut until repaired`}
                </b>
              </div>
              <div className="g-tf">
                <label>Policy</label>
                <b>none this season</b>
              </div>
            </div>
          </div>
          ...
          {g.configured && don.deedId !== 0n && tier === 0 && (
            <div style={{ textAlign: "center", margin: "10px 0" }}>
              <button
                className="g-cta-btn"
                disabled={busy !== null || don.away}
                onClick={() => run("upgrade")}
              >
                {busy === "upgrade"
                  ? "FILING…"
                  : `UPGRADE TO ROW HOUSE · ◫ ${fmtAmt(ROW_HOUSE_FEE)}`}
              </button>
              <p className="g-provnote" style={{ marginTop: 4 }}>
                burned from the vault · cap ◫ 15,000 · HD 60 · 3 chairs
              </p>
            </div>
          )}

          <hr className="g-dashrule" />
          <p className="g-pcap-h">THE THREE BOXES — where your Scrip can sit</p>
          <div className="g-threebox">
            <div className="g-tb vault">
              <h5>🔒 VAULT</h5>
              <div className="g-amt">
                {g.configured ? `◫ ${fmtAmt(don.vaultScrip)}` : "—"}
              </div>
              <div className="g-krow">
                <span>earns</span>
                <b>0</b>
              </div>
              <div className="g-krow">
                <span>risk</span>
                <b className="ok">never</b>
              </div>
            </div>
            <div className="g-tb">
              <h5>▤ BROKER</h5>
            ...
            <div className="g-tb house">
              ...
              <div className="g-amt">
                {g.configured
                  ? `◫ ${fmtAmt(don.deployedScrip + don.hopperScrip)}`
                  : "—"}
              </div>
              <div className="g-krow">
                <span>hopper</span>
                <b>{g.configured ? `◫ ${fmtAmt(don.hopperScrip)}` : "—"}</b>
              </div>
              <div className="g-krow">
                <span>risk</span>
                <b className="hot">robbery</b>
              </div>
            </div>
          </div>
          <p className="g-note">
            [ bank ⇄ deploy ] moving Scrip between boxes is one transaction each
            way
          </p>
          <div className="g-moverow">
            <input
              className="g-input"
              inputMode="decimal"
              placeholder="Scrip amount"
              value={amt}
              onChange={(e) => setAmt(e.target.value)}
              disabled={!!moveGate || busy !== null}
              aria-label="Scrip amount"
            />
            <button
            ...
            <button
              ...
              disabled={
                !!moveGate ||
                busy !== null ||
                (don.deployedScrip === 0n && don.hopperScrip === 0n)
              }
              onClick={() => run("bank")}
            >
              {busy === "bank" ? "…" : "BANK ALL"}
          ...
          {moveGate && (
            <p className="g-provnote" style={{ marginTop: 4 }}>
              {moveGate}. banking costs no fee and is never blocked once your
              Don is home.
            </p>
          )}
          ...
          <div className="g-cork">
            {damage > 0 && !repaired ? (
              <div className="g-corkcard">
                <span className="t">recent</span>
                <b className="bad">HIT</b> — earning rate −
                {Math.round(damage / 100)}% until repaired
                <button
                  className="g-stampbtn"
                  style={{ marginLeft: 6 }}
                  disabled={busy !== null}
              ...
              <div className="g-corkcard">
                ...
                <b className="okc">REPAIRED</b> — earning rate restored.{" "}
                <Stamp ok thump style={{ fontSize: 10, marginLeft: 6 }}>
                  PAID
                </Stamp>
              </div>
            ) : (
              <div className="g-corkcard">
                <span className="t">—</span>no incidents on file. this House has
                not been hit.
              </div>
            )}
            {!g.configured && (
              <div className="g-corkcard">
                <span className="t">soon</span>the log pins every attempt —{" "}
                <b className="okc">FAILED</b> or <b className="bad">HIT</b> —
                with its receipt, once the season opens.
              </div>
            )}
          </div>
