  ...
  return (
    <section className={cls} aria-label={tab} aria-hidden={!open}>
      ...
      <div className="g-sheet">
        <div className="g-wm" aria-hidden />
        <div className="g-punch p1" aria-hidden />
        <div className="g-punch p2" aria-hidden />
        <div className="g-filehead">
          <span>{head}</span>
          <span className="g-rule" />
          <button className="g-xbtn" onClick={onClose} aria-label="close file">
            ✕
          </button>
        </div>
        {(open || under) && children}
...
export function WaxSeal({
  ...
  return (
    <div className="g-sealwrap">
      <button
        className={
          "g-seal" + (pressing ? " pressing" : "") + (sealed ? " done" : "")
        }
...
export function DonImg({
  ...
  if (!ok)
    return (
      <span
        className={(className ? className + " " : "") + "g-imgph"}
        aria-hidden
      >
        №{id.toString()}
      </span>
  ...
  return (
    <img
      className={className}
      src={`/api/don-img/${id.toString()}`}
      alt={alt ?? `Don №${id.toString()}`}
      loading="lazy"
      decoding="async"
      onError={() => setOk(false)}
    />
...
function Blocks({ pct }: { pct: number }) {
  ...
  return (
    <span className="blocks">
      <b>{"▮".repeat(Math.min(10, n))}</b>
      {"░".repeat(Math.max(0, 10 - n))}
    </span>
  );
}
  ...
  return (
    <div className="g-island">
      <div className="g-cap">
        ...
        <span>{capRight ?? "published on-chain before you pay"}</span>
      </div>
      {rows.map(([k, cls, pct]) => (
      ...
      <div className="g-payline">
        <span>payout, marked</span>
        <b>{payout}</b>
      </div>
    </div>
  );
...
export const PaperClip = () => (
  <svg className="g-pclip" viewBox="0 0 22 44" aria-hidden>
    <path
      d="M11 3 c4 0 6.5 2.5 6.5 6 v24 c0 5-3 8-6.5 8 s-6.5-3-6.5-8 V12 c0-3 2-5 4.5-5 s4.5 2 4.5 5 v20"
      fill="none"
      stroke="#8f8a7c"
      strokeWidth="2.2"
      strokeLinecap="round"
  ...
  return (
    <div className="g-mug">
      ...
      <svg viewBox="0 0 100 100" aria-hidden>
        <path
          d="M50 10 c10 0 16 7 16 17 0 6-3 11-7 14 13 4 22 13 24 32 l1 27 H16 l1-27 c2-19 11-28 24-32 -4-3-7-8-7-14 0-10 6-17 16-17z"
          fill="#17140E"
        />
      </svg>
    ...
    <div className="g-chairslot">
      <div className="g-occ">
        <svg viewBox="0 0 44 52" aria-hidden>
          <path
            d="M8 2 h4 v26 h20 V2 h4 v34 h-4 v-4 H12 v4 H8z M10 36 l-3 14 h3 l3-12 M34 36 l3 14 h-3 l-3-12"
            fill={empty ? "none" : "#6b5b3d"}
            stroke={empty ? "#6b5b3d" : undefined}
            strokeWidth={empty ? 2 : undefined}
        ...
        {!empty && (
          <svg className="g-sil" viewBox="0 0 100 100" aria-hidden>
            <path
              d="M50 12 c9 0 14 7 14 15 0 6-2 10-6 13 12 3 20 12 22 28 l1 20 H19 l1-20 c2-16 10-25 22-28 -4-3-6-7-6-13 0-8 5-15 14-15z"
              fill="#17140E"
            />
          </svg>
        )}
