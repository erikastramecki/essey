// SOLVENCY SCAN — the D.O.N. game desk, on the game chain (TESTNET 46630, live.ts NET). It lives in
// the game wing and is gated with it; /explorer is the protocol explorer and reads mainnet 4663.
// THE WIRE (live event feed, world-voice lines, stamps) leads; THE STREET (who is exposed right now),
// FAMILIES (per-Don dossiers), THE FLOOR (the economy dashboard) drill down; THE RAILS is the
// game-era lending demo — pool, markets, open loans, firehose.
//
...
import { gameConfigured } from "./gameChain";
import { DonImg } from "./bits";
import { fmtHMS } from "./useGame";
import {
...
.txp .stamp.ok{color:var(--good)} .txp .stamp.mut{color:var(--tx-faint)} .txp .stamp.carbon{color:#4A5A78}
...
export function GameExplorerPage() {
  ...
  useEffect(() => { document.title = "Solvency Scan · D.O.N."; mounted.current = true; return () => { mounted.current = false; }; }, []);
