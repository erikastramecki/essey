...
import { garrisonCommit, buildLoadout, signLoadout, randomSalt } from "./loadout";
...
// ---------------------------------------------------------------------------------------------
export function JobBoardFile({ open, under, onClose }: { open: boolean; under: boolean; onClose: () => void }) {
    ...
    <CaseFile open={open} under={under} tab="THE JOB BOARD · FOLDER DRAWER" head="DISPATCH · OPEN POSTINGS" onClose={onClose}>
      <div className="g-drawer">
        {BRIEF_ORDER.map((k) => {
          const j = BRIEFS[k];
          return (
            <button key={k} className={"g-jobfolder" + (j.featured ? " featured" : "")} data-tab={j.tab}
              onClick={() => navigate(`/game/jobs/${k}`)}>
              <div className="g-jf-top">
                <span className="g-jf-code">{j.code}{j.featured && <i> ★</i>}</span>
                <span className="g-jf-take">▸ <b>◫ {fmtAmt(j.successPay)}</b> · {j.durationH}h</span>
              </div>
              <div className="g-jf-mid">
                <span className="g-jf-risk">risk {Array.from({ length: 8 }, (_, i) => i < j.riskBlocks
                  ? <b key={i}>▮</b> : <span key={i}>▮</span>)}</span>
        ...
        <div className="g-jobfolder locked" data-tab="the exchange raid · float squeeze" aria-disabled>
          <div className="g-jf-top">
            <span className="g-jf-code">SHORT FUSE</span>
            <span className="g-jf-take">▸ <b>GME</b> · 5h</span>
          </div>
          <div className="g-jf-mid">
            <span className="g-jf-risk">risk <b>▮▮▮▮▮▮</b>▮▮</span>
            <RatingStamp rating="steep">STEEP</RatingStamp>
          </div>
          <div className="g-strap">REQUIRES LEVEL 9</div>
        </div>
      </div>
      <p className="g-note" style={{ marginTop: 14 }}>the dispatcher never lies. if the odds are bad, THE WORD says so.</p>
    </CaseFile>
  );
}
...
export function DossierFile({ briefKey, open, onClose }: { briefKey: BriefKey | null; open: boolean; onClose: () => void }) {
  const g = useGame();
  ...
  useEffect(() => {
    if (open && brief) { setProvIdx(0); setPhase("idle"); setErr(null); setDepartedAt(null); }
  ...
  const depart = async () => {
    ...
    try {
      ...
      setPhase("submitting");
      await gameSend(g.address, GAME_ADDR.missionBoard, missionBoardAbi, "depart", [loadout, sig]);
      const now = new Date();
      setDepartedAt(`${String(now.getUTCHours()).padStart(2, "0")}:${String(now.getUTCMinutes()).padStart(2, "0")} UTC`);
      setPhase("departed");
      g.refresh();
    } catch (e) {
      setPhase("error");
  ...
  return (
    <CaseFile open={open} stack2 tab={`OPERATION BRIEF · №${brief.no}`} head="CASE FILE" onClose={onClose}>
      <div className="g-canon">
        {departed && <Stamp corner thump>DEPARTED</Stamp>}
        <div className="g-photowin">
          <PaperClip />
          <div className="g-pframe">
            {don
              ? <DonImg id={don.id} className="g-pimg" alt={`Don №${don.id.toString()} — the operative`} />
              : <span className="g-pimg g-imgph">NO FILE</span>}
          </div>
          <div className="g-pcap">subject · {don ? `don №${don.id.toString()}` : "unassigned"}</div>
        </div>
        <div className="g-typedfields">
          <p className="g-serifname">{brief.code}</p>
          <div className="g-tf"><label>Archetype</label><b>{brief.arch}</b></div>
          <div className="g-tf"><label>Operative</label><b>{don ? `DON №${don.id.toString()}` : "— none on desk"}</b></div>
          <div className="g-tf"><label>Clearance</label><b className={don ? "ok" : undefined}>{don ? "clears ✓" : "no operative"}</b></div>
          <div className="g-tf"><label>Rating</label><b>{brief.ratingLabel}{brief.rating === "unins" ? " — the Fixer declines" : " (the Fixer quotes on request)"}</b></div>
        </div>
      </div>
      ...
      <div className="g-typed">
        <p><span className="lbl">THE JOB</span>{brief.job}</p>
        <p><span className="lbl">THE TAKE</span>{brief.takeLine}</p>
        <p><span className="lbl">THE RISK</span>{brief.risk}</p>
        <p><span className="lbl">THE WORD</span>{brief.word}</p>
      </div>

      <hr className="g-dashrule" />
      <OddsLadder odds={brief.odds}
        payout={`◫ ${fmtAmt(brief.partialPay, 1)} sideways · ◫ ${fmtAmt(brief.successPay + boost, 1)} clean`} />

      <p className="g-pcap-h" style={{ marginTop: 12 }}>DURATION — fixed by the posting</p>
      <div className="g-exposure">
        exposure window: {brief.durationH}h — your House is robbable while you're away. that sentence is always on the folder.
      </div>

      <p className="g-pcap-h" style={{ marginTop: 14 }}>PROVISIONING — burned at dispatch, boosts the clean take</p>
      <div className="g-provs">
        {PROV_STEPS.map((p, i) => {
          const amt = (brief.provisionCap * p.num) / p.den;
          return (
            <button key={p.label} aria-pressed={provIdx === i}
              onClick={() => { if (!departed && !busy) setProvIdx(i); }}>
              {p.label}{amt > 0n ? ` · ◫ ${fmtAmt(amt)}` : ""}
            </button>
          );
        })}
      </div>
      <p className="g-provnote">
        provisions are consumed win or lose — the gamble. success pays +◫ {fmtAmt(boost, 1)} on top of the posted take. odds never move.
      </p>
      ...
      {!departed ? (
        <WaxSeal disabled={!!gate || busy} sealed={departed} hint={hint} onCommit={depart} />
      ) : (
        <div className="g-departbar">
          <Stamp big thump>DEPARTED {departedAt}</Stamp>
          <p className="g-provnote" style={{ marginTop: 10 }}>
            folder is now the active case — read-only, reachable from the desk. the resolve arrives as a stamp, not a popup.
          </p>
        </div>
      )}
    </CaseFile>
