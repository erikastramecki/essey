...
export function SpreadStrip() {
  ...
  const rows = [...dons].sort((a, b) =>
    atHouse(b) > atHouse(a) ? 1 : atHouse(b) < atHouse(a) ? -1 : 0,
  );
  const top = rows[0];
  ...
  const worstNow = awayDons.reduce(
    (m, d) => (reachOf(d) > m ? reachOf(d) : m),
    0n,
  );
    ...
    <div className="g-ledgerstrip g-spread">
      ...
      {exposed === 0n ? (
        <p className="g-sp-note">
          nothing is on the table. every Scrip you hold is banked in a vault,
          where no raid can reach it. the House only earns once you deploy, and
          only what you deploy can be taken.
        </p>
      ) : (
        rows.map((d) => {
          ...
          return (
            ...
            >
              <div className="g-sp-head">
                ...
                <span className="amt">
                  ◫ {fmtAmt(house)} · {round(share)}%
                </span>
              </div>
              <div className="g-sp-bar" aria-hidden>
                <i
                  className="dep"
                  style={{ width: `${pct(d.deployedScrip, exposed)}%` }}
                />
                <i
                  className="hop"
                  style={{ width: `${pct(d.hopperScrip, exposed)}%` }}
                />
              </div>
              <div className="g-sp-legs">
                deployed ◫ {fmtAmt(d.deployedScrip)} · hopper ◫{" "}
                {fmtAmt(d.hopperScrip)} · vault ◫ {fmtAmt(d.vaultScrip)}
              </div>
            </button>
          );
        })
      ...
      <div className="g-lrow">
        ...
        <b>
          ◫ {fmtAmt(exposed)}
          {family > 0n
            ? ` · ${round(pct(exposed, family))}% of the family`
            : ""}
        </b>
      </div>
      <div className="g-lrow">
        <span>IN THE VAULTS</span>
        <b>
          ◫ {fmtAmt(banked)} <span className="ok">never at risk</span>
        </b>
      </div>
      <div className="g-lrow">
        ...
        <b>
          ◫ {fmtAmt(worstAny)}
          {exposed > 0n ? (
            <span className="warnc">
              {" "}
              · {round(pct(worstAny, exposed))}% of the table
            </span>
          ) : null}
        </b>
      </div>
      ...
      <p className="g-sp-note">
        {exposed === 0n ? (
          <>
            a raid can only reach a House whose Don is away, and only what sits
            at that House. deploy when you want the yield, bank when you want it
            safe.
          </>
        ) : single ? (
          <>
            one Don is one House, so there is nothing to spread yet. everything
            you deploy sits behind the same door: a hit takes the whole hopper,
            a big score takes 15% to 30% of what is deployed, and both only
            while your Don is away. banking is your only cover here. a second
            Don is a second door, and every raid cap keys on the door it hits.
          </>
        ) : concentrated ? (
          <>
            <b>
              {round(topShare)}% of what is on the table sits behind Don №
              {top.id.toString()}.
            </b>{" "}
      ...
      {fatAway.length > 0 && (
        <p className="g-sp-note warn">
          {fatAway.length === 1 ? (
            <>
              Don №{fatAway[0].id.toString()} is in the field with ◫{" "}
              {fmtAmt(fatAway[0].hopperScrip)} unbanked. the hopper is the first
              thing a hit takes, and the escrow only moves while a Don is home.
              bank it the moment it lands.
            </>
          ) : (
            <>
              {fatAway.length} of your Dons are in the field with Scrip still in
              the hopper. the hopper is the first thing a hit takes, and the
              escrow only moves while a Don is home. bank each one the moment it
              lands.
            </>
          )}
        </p>
      ...
      <p className="g-sp-fine">
        the bands in full: a hit clears the hopper of the House it lands on,
        minus the 7.5% hit tax that burns. a big score takes between{" "}
        {Number(BIG_SLICE_MIN_BPS) / 100}% and {Number(BIG_SLICE_MAX_BPS) / 100}
        % of that House's deployed Scrip. the vault is on neither path.
      </p>
    </div>
  );
}
