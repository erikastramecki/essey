...
const disposition = (
  h: GameHitter,
): { label: string; rating: "low" | "std" | "steep" } => {
  const now = Date.now();
  if (h.hospitalUntil > now)
    return {
      label: `HOSPITAL ${fmtHMS((h.hospitalUntil - now) / 1000)}`,
      rating: "steep",
    };
  if (h.lastAttemptAt > 0 && now - h.lastAttemptAt < HITTER_COOLDOWN_MS)
    return {
      label: `COOLDOWN ${fmtHMS((h.lastAttemptAt + HITTER_COOLDOWN_MS - now) / 1000)}`,
      rating: "std",
    };
...
export function CrewFile({
  ...
  const expandedSealed = g.hitters.find(
    (h) => h.id.toString() === (expandedId ?? g.hitters[0]?.id.toString()),
  )?.sealed;
  ...
  useEffect(() => {
    ...
    pub
      .readContract({
        address: GAME_ADDR.hitter,
        abi: hitterAbi,
        functionName: "favorOf",
        args: [BigInt(id)],
      })
      .then((b) => {
        if (live) setFavorBand(FAVOR_BANDS[Number(b)] ?? null);
      })
      .catch(() => {});
    return () => {
      live = false;
  ...
  const revealFavor = async (id: bigint) => {
    ...
    try {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        try {
          const stillSealed = await pub.readContract({
            address: GAME_ADDR.hitter,
            abi: hitterAbi,
      ...
      setErr(
        /AlreadyRevealed|already/i.test(msg)
          ? "the envelope is already open — the Favor below is the record"
          : msg,
      );
      g.refresh();
    } finally {
  ...
  const mintGate = !g.configured
    ? "the crews sign with the season — contracts pending"
    : !g.connected
      ? "connect a wallet to retain a crew"
      : !don
        ? "a crew signs to a family — take a seat at the Registry first"
        : don.vaultScrip < MINT_PRICE
          ? `the retainer is ◫ ${fmtAmt(MINT_PRICE)} — the vault is short. run jobs on the board and BANK the hopper at the house; PROOF OF WORK pays ◫ 236 clean`
          : null;
  const expanded =
  ...
  return (
    ...
    >
        ...
        <div className="g-crewstrip">
          {g.hitters.map((h) => {
            ...
            return (
              <div
                key={h.id.toString()}
                className={"g-hitcard" + (expanded?.id === h.id ? " on" : "")}
                onClick={() => setExpandedId(h.id.toString())}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
                  if (e.key === "Enter") setExpandedId(h.id.toString());
                }}
              >
                <RedactedMug />
                ...
                <div className={"g-sealedslot" + (h.sealed ? "" : " revealed")}>
                  <SealedEnvelopeIcon />{" "}
                  {h.sealed ? "THE FAVOR · SEALED" : "FAVOR REVEALED"}
                </div>
              </div>
            );
          })}
      ...
      <div style={{ textAlign: "center", margin: "12px 0 4px" }}>
        <button
          className="g-cta-btn"
          disabled={!!mintGate || busy !== null}
          onClick={mint}
          style={mintGate ? { opacity: 0.5, cursor: "not-allowed" } : undefined}
        >
          {busy === "mint"
            ? "SIGNING…"
            : `RETAIN A HITTER · ◫ ${fmtAmt(MINT_PRICE)}`}
        </button>
        {mintGate && (
      ...
      {expanded && (
        <>
          <p className="g-pcap-h" style={{ marginTop: 12 }}>
            EXPANDED SHEET — №{expanded.id.toString()}
          </p>
          <div className="g-rapsheet">
            <div className="g-canon">
              <Stamp corner style={{ fontSize: 10 }}>
                {disposition(expanded).label.split(" ")[0] === "COOLDOWN"
                  ? "COOLING OFF"
                  : disposition(expanded).label.split(" ")[0]}
              </Stamp>
              <div className="g-photowin" style={{ flexBasis: 96, width: 96 }}>
                <div className="g-pframe" style={{ paddingBottom: 8 }}>
                  <RedactedMug />
                </div>
              </div>
              <div className="g-typedfields">
                ...
                <div className="g-tf">
                  ...
                  <b>{disposition(expanded).label.toLowerCase()}</b>
                </div>
                <div className="g-tf">
                  <label>Carries</label>
            ...
            {expanded.sealed && g.connected && (
              <div style={{ marginTop: 8, textAlign: "center" }}>
                <button
                  ...
                  disabled={busy !== null}
                  onClick={() => revealFavor(expanded.id)}
                >
                  {busy === "reveal"
                    ? "SIGNING…"
                    : busy === "opening"
                      ? "THE WORD IS OUT — OPENING…"
                      : "OPEN THE ENVELOPE · REVEAL THE FAVOR"}
                </button>
              </div>
            )}
            <p
