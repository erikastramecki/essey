...
const GROUPS = ["The game", "The Market", "Essey Private", "The engine", "Audits"];
...
const LEARN_ITEMS: NavLeaf[] = [
  { to: "/docs", label: "Technical docs", desc: "The protocol underneath, from the repo's own files" },
...
const MORE_ITEMS: NavLeaf[] = [
  ...
  { to: "/bell", label: "The Bell", desc: "Where protocol fees pool and pay out" },
...
const MOBILE_NAV: [string, NavLeaf[]][] = [
  ["The city", [
    ...
    { to: "/how-to-play", label: "How to Play", desc: "the walkthrough, file by file" },
      ...
      <WalletProvider>
        ...
        <Routes>
          <Route element={<Layout />}>
            ...
            <Route path="/market" element={<AppPage title="The Exchange"><LiveExchange /></AppPage>} />
            ...
            <Route path="/how-to-play" element={<AppPage title="How to Play"><HowToPlayPage /></AppPage>} />
            {/* The Seats-era explainer is retired; old links land on the game walkthrough. */}
            <Route path="/how-it-works" element={<Navigate to="/how-to-play" replace />} />
...
function Landing() {
  const w = useWallet();
  useEffect(() => { document.title = "Essey · D.O.N. — the on-chain city where the odds and the books are both provable"; }, []);
      ...
      <section className="band" style={{ paddingTop: 8 }}>
        <div className="wrap">
          ...
          <div className="dest-grid">
            {[
              ["/game", "♠", "The Game", "The desk, the job board, the stakeout. Send your Don to work in the city of Solvency."],
              ["/how-to-play", "🗂", "How to Play", "The whole game, file by file: jobs, banking, raids, and every number from the chain."],
              ["/builder", "◇", "Mint", "Build your Don trait by trait, reroll a random one, or claim free on the whitelist."],
              ["/market", "⇄", "Trade", "Buy, snipe, or sell Dons at the floor-pinned price on the live on-chain desk."],
              ["/tape", "📈", "The Tape", "Everything the city does, printed live — every line a real transaction receipt."],
              ["/portfolio", "◈", "Portfolio", "Everything you hold: your Dons, their Vaults, and every action on them."],
            ].map(([to, icon, h, p]) => (
          ...
          <div className="learn-row">
            The rails underneath: <Link to="/provable">Provable</Link> · <Link to="/engine">The engine</Link> · <Link to="/docs">Technical docs</Link>
          </div>
        </div>
      </section>
  ...
  return (
    <footer>
      <div className="wrap foot-cols">
        {([
          ["The city", [["/game", "The Game"], ["/how-to-play", "How to Play"], ["/builder", "Mint"], ["/market", "Trade"], ["/portfolio", "Portfolio"]]],
          ["Learn", [["/docs", "Technical docs"], ["/provable", "Provable"], ["/engine", "The engine"]]],
          ["More", [["/faucet", "Faucet"], ["/tape", "The Tape"], ["/explorer", "Explorer"], ["/bell", "The Bell"], ["/private", "Private"], ["/launch", "Launch"]]],
      ...
      <div className="wrap foot-in">
        <div>
          <p className="disclaim"><b>This season settles in Scrip</b> — play points marked to tickers.
            Real-token settlement is the roadmap; nothing won in the game today is real stock. "Payout," never
            "dividend": protocol fee distributions are mechanically LP-style fee-shares, not dividends, not
            yield promises, and no payout is guaranteed, ever. The contracts are adversarially audited
            (published rounds in the technical docs) and <b>live on Robinhood Chain testnet</b>. Everything
            here is play money with no real value; not on mainnet.{" "}
...
function DocsPage() {
  ...
  return (
    <section className="band" id="docs">
      <div className="wrap">
        <div className="band-head"><div><span className="eyebrow">Technical docs</span><h2>Read the whole thing</h2>
          <p>The game's rulebook, the tokenomics, the lending engine's risk framework, every adversarial audit
            round, and the list of what is still unfinished. These are the repo's own files, rendered here, not
            a marketing summary of them.</p></div></div>
        {GROUPS.map((g) => {
