...
const SELECT: Record<G, [string, string, boolean][]> = {
  male: [
    ["4 Body", "Bloodline", false],
    ["5 Suit", "Suit", false],
    ["13 Hair", "Hair", false],
    ["17 Face Mod", "Face Mod", true],
    ["15 Eye Mod", "Eye Mod", true],
    ["16 Glasses", "Glasses", true],
    ["22 Hat", "Hat", true],
    ["23 Ceasar", "Crown", true],
    ["24 Laser Eye", "Laser", true],
    ["19 Hand Grip", "Hand", true],
    ["18 Canes", "Cane", true],
    ["25 Snake", "Snake", true],
    ["20 Rings", "Ring", false],
    ["21 Wrist", "Wrist", false],
    ["26 AR", "AR Screen", true],
    ["2 The hawk", "Hawk", true],
    ["1 Background", "Background", false],
    ["3 Chair", "Chair", false],
  ],
  female: [
    ["3 Body", "Bloodline", false],
    ["4 Suits", "Suit", false],
    ["9 Hair", "Hair", false],
    ["8 Necklace", "Necklace", false],
    ["10 Hat", "Hat", true],
    ["11 Devilish", "Devilish", true],
    ["16 Neko", "Neko Mask", true],
    ["15 Earing", "Earring", false],
    ["13 Hand Grip", "Hand", true],
    ["14 Ring", "Ring", false],
    ["12 Wrist dec", "Wrist", false],
    ["17 AR", "AR Screen", true],
    ["18 phoenix eyes", "Phoenix Eyes", true],
    ["1 Chairs", "Chair", false],
  ],
};
  ...
  try {
    ...
    await eth.request({
      ...
      params: [
        {
          chainId: hexId,
          chainName: donChain.name,
          nativeCurrency: donChain.nativeCurrency,
          rpcUrls: [DON_NET.rpc],
          blockExplorerUrls: [DON_NET.explorer],
        },
      ],
    });
    await eth.request({
...
function friendly(e: any): string {
  ...
  if (has("ComboTaken"))
    return "Someone got this exact 1-of-1 first. Tweak a trait for a fresh combo.";
  ...
  if (has("StakedNoReroll") || has("TraitsLocked"))
    return "Staked art is locked: staking freezes a Don's look forever.";
  if (has("BadProof"))
    return "Allowlist root isn't committed on-chain yet. Claims open soon.";
  if (has("StageClosed")) return "This claim stage isn't open yet.";
  if (has("AlreadyClaimed"))
    return "This wallet already claimed its allocation.";
  if (has("LienActive"))
    return "This Don has an active loan lien. Repay first.";
...
// The seed is a random uint32, so a preimage that never reaches the registry is UNRECOVERABLE and the
// Don renders a placeholder forever. A fire-and-forget version lost 11 Dons during an API outage.
const REVEAL_QUEUE = "essey.pendingReveals";
type RevealSel = {
  gender: string;
  forced: Record<string, string>;
  seed: number;
};
type RevealBody = RevealSel & { id?: number };

function readQueue(): RevealBody[] {
  try {
    return JSON.parse(localStorage.getItem(REVEAL_QUEUE) || "[]");
  } catch {
    return [];
  }
}

function writeQueue(q: RevealBody[]) {
  try {
    localStorage.setItem(REVEAL_QUEUE, JSON.stringify(q.slice(-50)));
  } catch {}
}

async function postReveal(body: RevealBody, attempts = 4): Promise<boolean> {
  for (let i = 0; i < attempts; i++) {
    try {
      const r = await fetch("/api/don-reveal", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(body),
      });
      if (r.ok) return true;
      if (r.status === 404) return false; // combo genuinely not on-chain; retrying cannot help
    } catch {}
    await new Promise((res) => setTimeout(res, 400 * 2 ** i));
  }
  const dupe = (q: RevealBody) => body.id !== undefined && q.id === body.id;
  writeQueue([...readQueue().filter((q) => !dupe(q)), body]);
  return false;
}

async function drainPendingReveals() {
  const q = readQueue();
  if (!q.length) return;
  const left: RevealBody[] = [];
  for (const body of q) if (!(await postReveal(body, 2))) left.push(body);
  writeQueue(left);
}
...
export function BuilderPage() {
  ...
  useEffect(() => {
    (["male", "female"] as G[]).forEach((g) => {
      if (!data[g])
        fetch(`/builder/data_${g}.json`)
          .then((r) => r.json())
          .then((d: BuilderData) => setData((p) => ({ ...p, [g]: d })))
          .catch(() => {});
    });
  }, []); // eslint-disable-line

  const [rsv, setRsv] = useState<{
    status: "idle" | "busy" | "reserved" | "taken" | "offline" | "error";
    by?: string;
  }>({ status: "idle" });
  const d = data[gender];
  const opts = useMemo(() => (d ? catOptions(d) : {}), [d]);
  const blocked = useMemo(
    () => (d ? blockedCats(d, picks, seed) : new Set<string>()),
    [d, picks, seed],
  );
  ...
  const deadCats = useMemo(() => {
    ...
    for (const [cat] of SELECT[gender]) {
      const sel = (opts[cat] || []).filter(
        (o) => o && o.toLowerCase() !== "none",
      );
      const u = unavail.get(cat);
      if (sel.length > 0 && u && sel.every((o) => u.has(o)))
        s.set(cat, u.get(sel[0]) || "");
    }
    return s;
  }, [unavail, opts, gender]);
  ...
  useEffect(() => {
    setRsv({ status: "idle" });
    setMint((m) =>
      m.status === "error" || m.status === "success" ? { status: "idle" } : m,
    );
    setRerollTx((m) =>
      m.status === "error" || m.status === "success" ? { status: "idle" } : m,
    );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      pub.readContract({
        address: DON_NET.distributor,
        abi: distributorAbi,
        functionName: "rerollFee",
      }),
      pub.readContract({
        address: DON_NET.distributor,
        abi: distributorAbi,
        functionName: "publicOpen",
      }),
    ])
      .then(([customFee, rerollFee, publicOpen]) =>
  ...
  const loadOwned = useCallback(async (addr: `0x${string}`) => {
    try {
      ...
      const ids = Array.from({ length: Math.min(total, 500) }, (_, i) =>
        BigInt(i + 1),
      );
      ...
      setOwned(
        ids
          .filter((_, i) => owners[i]?.toLowerCase() === addr.toLowerCase())
          .map((id) => Number(id)),
      );
    } catch {
      setOwned([]);
    ...
    try {
      ...
      const [claimed, root, open] = await Promise.all([
        ...
        pub.readContract({
          address: DON_NET.distributor,
          abi: distributorAbi,
          functionName: "stageRoot",
          args: [stage],
        }),
        pub.readContract({
          address: DON_NET.distributor,
          abi: distributorAbi,
          functionName: "stageOpen",
          args: [stage],
        }),
      ]);
      setWl({
  ...
  const connect = useCallback(async (): Promise<`0x${string}`> => {
    ...
    if (!eth)
      throw new Error(
        "No EVM wallet found. Install MetaMask or Rabby to mint.",
      );
    ...
    setWallet(addr);
    loadWl(addr);
    loadOwned(addr); // fire-and-forget context loads
    return addr;
  }, [loadWl, loadOwned]);
  ...
  const connectClick = useCallback(async () => {
    try {
      ...
      setMint({ status: "error", msg: friendly(e) });
    }
  }, [connect]);

  useEffect(() => {
    // wallet account switches reset the session context (bound to the SELECTED wallet)
    const eth = sel as any;
    if (!eth?.on) return;
  ...
  const reserve = useCallback(async () => {
    ...
    try {
      ...
      if (!r.ok) {
        setRsv({ status: "offline" });
        return;
      } // 503 store-not-provisioned etc — never block the mint
  ...
  const registerReveal = useCallback((sel: RevealSel, id?: number) => {
    void postReveal({ ...sel, id });
  }, []);
  ...
  const doMint = useCallback(async () => {
    ...
    try {
      ...
      if (used) {
        setMint({
          status: "error",
          msg: "Someone got this exact 1-of-1 first. Tweak a trait for a fresh combo.",
        });
        return;
      }
      if (!open) {
        setMint({
          status: "error",
          msg: "Public custom minting isn't open yet.",
        });
        return;
      }
      ...
      if (rcpt.status !== "success") {
        setMint({ status: "error", msg: "Transaction reverted on-chain." });
        return;
      }
  ...
  const doClaimWl = useCallback(async () => {
    ...
    try {
      ...
      while (rolls.length < wl.allocation && tries < 600) {
        ...
        if (!seen.has(k)) {
          seen.add(k);
          rolls.push({ forced: f, seed: s, key: k });
        }
        tries++;
      }
      if (rolls.length < wl.allocation) {
        setWlTx({
          status: "error",
          msg: "Couldn't roll enough distinct combos. Clear a few picks and retry.",
        });
        return;
      }
      ...
      if (used.some(Boolean)) {
        setWlTx({
          status: "error",
          msg: "A rolled combo collided. Hit claim again for a fresh roll.",
        });
        return;
      }
      const { request } = await pub.simulateContract({
        ...
        args: [BigInt(wl.stage), BigInt(wl.allocation), wl.proof, combos],
        account: wallet,
      });
      const hash = await walletClient().writeContract(request);
      ...
      const rcpt = await pub.waitForTransactionReceipt({ hash });
      if (rcpt.status !== "success") {
        setWlTx({ status: "error", msg: "Transaction reverted on-chain." });
        return;
      }
      ...
      if (firstId !== undefined)
        rolls.forEach((r, i) =>
          registerReveal(
            { gender, forced: r.forced, seed: r.seed },
            firstId! + i,
          ),
        );
      setWlTx({
        ...
        hash,
        id: firstId,
        note: `${wl.allocation} Don${wl.allocation > 1 ? "s" : ""} minted`,
      });
      setWl({ ...wl, claimed: true });
      loadOwned(wallet);
    } catch (e: any) {
      setWlTx({ status: "error", msg: friendly(e) });
    }
  }, [
    res,
    wl,
  ...
  const doReroll = useCallback(async () => {
    ...
    try {
      ...
      if (used) {
        setRerollTx({
          status: "error",
          msg: "That exact combo is already minted. Tweak a trait first.",
        });
        return;
      }
      if (locked) {
        setRerollTx({
          status: "error",
          msg: "Staked art is locked: staking freezes a Don's look forever.",
        });
        return;
      }
      ...
      const rcpt = await pub.waitForTransactionReceipt({ hash });
      if (rcpt.status !== "success") {
        setRerollTx({ status: "error", msg: "Transaction reverted on-chain." });
        return;
      }
      ...
      setRerollTx({
        ...
        hash,
        id: rerollSel,
        note: `Don #${rerollSel} rerolled`,
      });
    } catch (e: any) {
      setRerollTx({ status: "error", msg: friendly(e) });
    }
  ...
  const switchGender = (g: G) => {
    setGender(g);
    setPicks({});
  };

  const wlEtaHrs = Math.max(
    0,
    Math.ceil((WL_STAGE0_COMMIT_ETA_MS - Date.now()) / 36e5),
  );
      ...
      <div className="bld-grid">
        ...
        <div className="bld-stage">
          ...
          <div className="bld-row">
            {(["male", "female"] as G[]).map((g) => (
              <button
                key={g}
                className={"bld-chip" + (gender === g ? " on" : "")}
            ...
            <button className="bld-chip" onClick={clearAll}>
              Clear
            </button>
          </div>
          <div className="bld-key">
            <span>Uniqueness key</span>
            <br />
          ...
          </div>
          {
            <>
              ...
              <button
                onClick={reserve}
                disabled={rsv.status === "busy" || rsv.status === "reserved"}
                className={
                  "bld-chip" +
                  (rsv.status === "reserved"
                    ? " good"
              ...
              >
                {rsv.status === "reserved"
                  ? "✓ Held for 30 min, mint below"
                  : rsv.status === "taken"
                    ? "✗ Held by someone, change a trait"
                    : rsv.status === "busy"
                      ? "Holding…"
                      : rsv.status === "offline"
                        ? "Holds offline, mint is still first-come"
                        : "Hold this combo (optional, 30 min)"}
              </button>
              {rsv.status === "taken" && (
                <div className="bld-hint">
                  This exact combination is on hold or already minted. Change
                  any trait for a fresh 1-of-1.
                </div>
              )}
              {rsv.status === "offline" && (
                <div className="bld-hint">
                  The soft-hold service is offline. Nothing is lost; minting
                  stays first-come-first-served on-chain.
                </div>
              )}

              {/* custom mint */}
              <div className="bld-hint">
                This exact combination is checked against the on-chain
                uniqueness ledger. Once you mint it, no other Don can ever share
                it. Custom mint costs {fmtEth(fees.customFee)} ETH, and 100% of
                the fee buys stock for staked holders.
              </div>
              <button
                onClick={doMint}
                disabled={busyAny}
                className={
                  "btn bld-mint " +
                  (mint.status === "success" ? "bld-chip good" : "btn-gold")
                }
              ...
              >
                {mint.status === "busy"
                  ? mint.note || "Preparing…"
                  : mint.status === "pending"
                    ? "Minting: confirm in wallet / waiting for chain…"
                    : mint.status === "success"
                      ? `✓ Minted${mint.id !== undefined ? ` Don #${mint.id}` : ""}`
                      : wallet
                        ? `Mint this 1-of-1 · ${fmtEth(fees.customFee)} ETH`
                        : "Connect & mint this 1-of-1"}
              </button>
              {fees.publicOpen === false && mint.status === "idle" && (
                <div className="bld-hint">
                  Public custom minting isn't open yet. The button will
                  preflight again when you try.
                </div>
              )}
              {(mint.status === "pending" || mint.status === "success") && (
                <div className="bld-hint">
                  {mint.status === "success" && mint.id !== undefined && (
                    <>
                      Don #{mint.id} is yours. This combo is retired
                      forever.{" "}
                    </>
                  )}
                  <a href={txUrl(mint.hash)} target="_blank" rel="noreferrer">
                    View on explorer ↗
                  </a>
                </div>
              )}
              {mint.status === "error" && (
                <div className="bld-hint err">{mint.msg}</div>
              )}
              ...
              {wallet && wl && (
                <div className="bld-panel">
                  ...
                  {wl.claimed ? (
                    <div className="bld-hint">
                      ✓ This wallet already claimed its {wl.allocation} free Don
                      {wl.allocation > 1 ? "s" : ""}.
                    </div>
                  ) : !wl.rootLive || !wl.open ? (
                    <div className="bld-hint">
                      You're on the list: {wl.allocation} free Don
                      {wl.allocation > 1 ? "s" : ""}. Claims open once the
                      allowlist root commits on-chain
                      {wlEtaHrs > 0
                        ? ` (~${wlEtaHrs}h, after the 2-day timelock)`
                        : " (any moment now)"}
                      .
                    </div>
                  ) : (
                    <>
                      <div className="bld-hint">
                        You're on the list: {wl.allocation} free Don
                        {wl.allocation > 1 ? "s" : ""}. Your whitelist
                        allocation mints for gas only. The first uses your
                        crafted combo, the rest roll random traits. You can
                        reroll the look as many times as you like before you
                        stake; staking locks it forever.
                      </div>
                      <button
                        onClick={doClaimWl}
                        disabled={
                      ...
                      >
                        {wlTx.status === "busy"
                          ? wlTx.note || "Preparing…"
                          : wlTx.status === "pending"
                            ? "Claiming: waiting for chain…"
                            : wlTx.status === "success"
                              ? `✓ ${wlTx.note}${wlTx.id !== undefined ? `, first is #${wlTx.id}` : ""}`
                      ...
                      {wlTx.status === "error" && (
                        <div className="bld-hint err">{wlTx.msg}</div>
                      )}
                    </>
                  )}
                </div>
              ...
              {wallet && owned.length > 0 && (
                <div className="bld-panel">
                  ...
                  <div className="bld-hint">
                    Re-randomize this Don's traits for {fmtEth(fees.rerollFee)}{" "}
                    ETH, unlimited, until it's staked. Your old combo is
                    released back to the pool and the fee buys stock for staked
                    holders, all of it. The current preview becomes the Don's
                    new look.
                  </div>
                  <div className="bld-opts">
                    {owned.map((id) => (
                      <button
                        key={id}
                        className={"bld-opt" + (rerollSel === id ? " on" : "")}
                        onClick={() =>
                          setRerollSel(rerollSel === id ? null : id)
                        }
                      >
                        #{id}
                      </button>
                  ...
                  {rerollSel !== null && (
                    <>
                      <button
                        onClick={doReroll}
                        disabled={
                          rerollTx.status === "busy" ||
                          rerollTx.status === "pending"
                        }
                        className={
                          "bld-chip " +
                          (rerollTx.status === "success" ? "good" : "on")
                        }
                        style={{ padding: 11, fontSize: 13, marginTop: 8 }}
                      >
                        {rerollTx.status === "busy"
                          ? rerollTx.note || "Preparing…"
                          : rerollTx.status === "pending"
                            ? "Rerolling: waiting for chain…"
                            : rerollTx.status === "success"
                              ? `✓ ${rerollTx.note}`
                      ...
                      {rerollTx.status === "error" && (
                        <div className="bld-hint err">{rerollTx.msg}</div>
                      )}
                    </>
                  )}
                </div>
        ...
        <div className="bld-cats">
          {SELECT[gender].map(([cat, label, opt]) => {
            ...
            const selNone =
              typeof sel === "string" && sel.toLowerCase() === "none";
            ...
            const pickHidden =
              pick !== undefined && pick !== "none" && !!catUnavail?.has(pick);
            if (collapsed) {
              return (
                <div
                  key={cat}
                  className="bld-cat collapsed"
                ...
                >
                  <div className="bld-cat-top">
                    ...
                    <span className="cs">
                      {deadReason}
                      {pickHidden ? " · your pick is hidden" : ""}{" "}
                      <span className="bld-exp">＋</span>
                    </span>
                  </div>
                </div>
              );
              ...
              <div key={cat} className="bld-cat">
                <div className="bld-cat-top">
                  ...
                  {deadReason !== undefined ? (
                    <span
                      className="cs bld-exp-btn"
                      onClick={() => toggleExpand(cat)}
                    >
                      {deadReason} <span className="bld-exp">－</span>
                    </span>
                  ) : cat === "13 Hair" && res?.drivers.hat ? (
                    <span className="cs">under hat: sets beard/brows</span>
                  ) : catUnavail && catUnavail.size > 0 ? (
                    <span className="cs">some options unavailable</span>
                  ) : sel && !selNone ? (
                    <span className="cs cv">{clean(sel)}</span>
                  ) : null}
                </div>
                <div className="bld-opts">
                  {opt && (
                    <button
                      className={"bld-opt" + (selNone ? " on" : "")}
                      onClick={() => setPick(cat, "none")}
                    >
                      ∅ None
                    </button>
                  )}
