...
// paid. The anonymity it gives grows with how many people use it. Not yet live on mainnet.
...
export function PrivatePage() {
  ...
  const [pool, setPool] = useState<{ notes: PoolNote[]; balance: bigint }>({
    notes: [],
    balance: 0n,
  });
  ...
  const [relayFallback, setRelayFallback] = useState<{
    noun: string;
  ...
  const maxNote = pool.notes.reduce(
    (m, n) => (BigInt(n.amount) > m ? BigInt(n.amount) : m),
    0n,
  ); // largest single note (1 note/tx)
  ...
  useEffect(() => {
    if (a && !unshieldTo) setUnshieldTo(a);
  }, [a, unshieldTo]);
  useEffect(() => {
    scanGen.current++;
    setPoolKeys(null);
    setPool({ notes: [], balance: 0n });
    setUsdgValue(null);
    setImpaired(false);
    setMaxNoteUsdg(null);
    setPoolReg(null);
    setRelayFallback(null);
  }, [a]); // re-lock on wallet change
  ...
  useEffect(() => {
    ...
    lookupStealthMeta(a)
      .then((m) => {
        if (live) setRegistered(!!m && m.length > 2);
      })
      .catch(() => {
        if (live) setRegistered(null);
      });
    return () => {
      live = false;
    };
  ...
  const maxHint = isSupply
    ? `≈ ${fmt(maxNoteUsdg ?? 0n, 2)} USDG`
    : `${fmt(maxNote, selPool.unit === "USDG" ? 2 : 4)} ${selPool.unit}`;
  ...
  const pickNote = (raw: bigint): PoolNote => {
    const note = [...pool.notes].sort((x, y) =>
      BigInt(y.amount) > BigInt(x.amount) ? 1 : -1,
    )[0];
    if (!note) throw new Error("Nothing shielded yet.");
    if (raw > BigInt(note.amount))
      throw new Error(
        `That's more than your largest single note (${maxHint}). Each transaction spends one note at a time, so withdraw/send in steps.`,
      );
    return note;
  };
    ...
    run(
      ...
      async () => {
        const amt = parseUnits(
          shieldAmt || "0",
          isSupply ? USDG_DECIMALS : selPool.decimals,
        );
        if (amt <= 0n) throw new Error("Enter an amount to shield.");
        await flows.shieldDeposit(a, amt, poolKeys, selPool, setStage);
        setShieldAmt("");
        await rescan(poolKeys, selPool);
      },
      isSupply
        ? "✓ Supplied privately. Your position and its yield are now hidden."
  ...
  const unshield = () =>
    ...
    runRelayable(
      ...
      isSupply
        ? "✓ Withdrawn. USDG plus accrued yield sent to your address."
  ...
  const transfer = () =>
    ...
    runRelayable(
      ...
      async () => {
        ...
        if (!isAddress(dest))
          throw new Error("Enter the recipient's wallet address.");
        await flows.shieldTransfer(
          a,
          dest as Address,
          raw,
          pickNote(raw),
    ...
    run(
      ...
      async () => {
        ...
        if (isAddress(t)) {
          ...
          if (!m || m.length <= 2)
            throw new Error(
              "That address hasn't set up a private receiving address yet.",
            );
          meta = m;
          ...
          throw new Error(
            "Enter the recipient's wallet address, or their 66-byte meta-address.",
          );
        }
        if (!amount || Number(amount) <= 0)
          throw new Error("Enter an amount to send.");
        const tok = TOKENS.find((t) => t.addr === token)!;
        const { stealthAddress } = await flows.payPrivate(
          a,
          token,
          meta,
          parseUnits(amount, tok.decimals),
        );
        setMsg(
          `✓ Sent privately to a one-time address (${short(stealthAddress)}). Only the recipient can find it.`,
        );
        setAmount("");
      },
      "",
  ...
  const sweep = (h: PrivateHolding, tokenAddr: Address) =>
    a &&
    keys &&
    run(
      "sweep" + h.stealthAddress + tokenAddr,
      async () => {
        const dest = (sweepTo.trim() || a) as Address;
        if (!isAddress(dest))
          throw new Error("Enter a valid sweep destination address.");
      ...
      <div className="wrap">
        <div className="band-head">
          <div>
            ...
            <p>
              Two ways to move without being watched: <b>shielded pools</b> that
              hide your balance and amounts (for USDG, AAPL/NVDA stock, and
              yield-bearing supply), and <b>stealth addresses</b> that hide who
              you're paid as. Both experimental, not yet live on mainnet.
            </p>
          </div>
        ...
        <div className="live-card" style={{ marginBottom: 16 }}>
          <div className="live-note" style={{ lineHeight: 1.5 }}>
            <b>What this is.</b> Two privacy tools. The <b>shielded pools</b>{" "}
            (below) hide amounts: your balance and in-pool transfers are
            private, and deposits can't be linked to withdrawals; pick USDG, a
            stock (AAPL/NVDA), or private yield-bearing supply.{" "}
            <b>Stealth addresses</b> hide the link between your identity and
            where you're paid (amounts there stay public). Both grow stronger
            the more people use them. Experimental — not yet live on mainnet.
          </div>
        </div>
        ...
        {!connected ? (
          <div className="live-card">
            <div className="live-row">
              <span className="live-note">
                Connect your wallet to use Essey Private.
              </span>
              <ConnectButton />
            </div>
          </div>
          ...
          <>
            <div className="pf-note" style={{ marginBottom: 12 }}>
              New here? Every action needs a little <b>gas ETH</b>, and
              shielding needs <b>play-money tokens</b>. Grab both free on the{" "}
              <Link className="pf-link gold" to="/faucet">
                Faucet ⚡
              </Link>
              .
            </div>
            ...
            <div className="pf-block">
              <div className="pf-block-h">
                Shielded balance{" "}
                <span className="preview-chip live">hides amounts</span>
              </div>

              {/* Pool selector — one section over four pools. Keys are shared, so switching just rescans. */}
              <div className="pf-note" style={{ marginBottom: 6 }}>
                Choose what to make private:
              </div>
              <div
                className="live-row"
                ...
                {SHIELDED_POOLS.map((p) => (
                  <button
                    ...
                    style={{
                      padding: "6px 12px",
                      fontSize: 13,
                      opacity: p.key === selPoolKey ? 1 : 0.5,
                      borderColor:
                        p.key === selPoolKey
                          ? "var(--gold, #c9a227)"
                          : undefined,
              ...
              {!poolKeys ? (
                <div className="live-card">
                  ...
                  >
                    <span className="live-note" style={{ flex: "1 1 320px" }}>
                      {selPool.blurb} Sign once to unlock:{" "}
                      <b>
                        no gas, no approvals, nothing spent, just a signature
                      </b>
                      . The same keys work across every pool, and your notes are
                      recovered by scanning the chain, so they follow you across
                      devices (and include anything others have sent you).
                    </span>
                    <button
                      className="btn btn-gold"
                      disabled={busy === "unlockPool"}
                      onClick={unlockPool}
                    >
                      {busy === "unlockPool"
                        ? stage
                ...
                <div className="live-card">
                  {isSupply ? (
                    <div style={{ marginBottom: 4 }}>
                      ...
                      <div className="pf-note">
                        Private supply · earning yield · {fmt(pool.balance, 2)}{" "}
                        shares held
                      </div>
                    </div>
                  ) : (
                    ...
                    >
                      {fmt(pool.balance, selPool.unit === "USDG" ? 2 : 4)}{" "}
                      <span
                        style={{ fontSize: 15, opacity: 0.6, fontWeight: 400 }}
                      >
                        {selPool.unit} shielded
                      </span>
                    </div>
                  )}
                  <div className="pf-note" style={{ marginBottom: 14 }}>
                    {isSupply ? (
                      <>
                        Your supply position and the yield it earns are hidden.
                        The note's share count is fixed, but the shares
                        appreciate as interest accrues. Withdraw any time to
                        receive USDG plus accrued yield.
                      </>
                    ) : (
                      <>
                        Your balance and any in-pool transfer are hidden, and
                        the pool breaks the deposit↔withdrawal link.
                        Deposit/withdraw <b>amounts</b> are public (matching
                        amounts can re-link on a small pool, so unshield to a
                        fresh address).
                      </>
                    )}
                    <button
                      className="pf-link gold pf-inline-btn"
                      disabled={!!busy}
                      onClick={() => {
                        if (poolKeys)
                          run(
                            "rescan",
                            () => rescan(poolKeys, selPool),
                            "✓ Rescanned.",
                          );
                      }}
                    >
                  ...
                  {pool.notes.length > 1 && (
                    <div className="pf-note" style={{ marginBottom: 14 }}>
                      Your balance is made of separate deposits ("notes"); each
                      transaction spends one at a time. Largest single
                      withdrawal or transfer: <b>{maxHint}</b>. Larger amounts
                      go in steps.
                    </div>
                  )}
                  ...
                  {selPool.stock && impaired && (
                    ...
                    >
                      <div className="pf-note" style={{ margin: 0 }}>
                        ⚠ <b>Backing impaired.</b> The issuer has burned some of
                        this pool's stock, so withdrawals now pay a{" "}
                        <b>pro-rata share</b> of what remains. The loss is split
                        fairly across all holders, in the same proportion no
                        matter when you exit. New deposits are closed until the
                        backing is restored.
                      </div>
                    </div>
                  )}
                  ...
                  {poolReg === false && (
                    <div className="live-card" style={{ marginBottom: 12 }}>
                      ...
                      >
                        ...
                        >
                          To <b>receive</b> shielded {selPool.label} from
                          others, publish your keys once (a small tx). Not
                          needed to shield/unshield your own funds.
                        </span>
                        <button
                          className="btn"
                          disabled={!!busy}
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={viaRelayer}
                      onChange={(e) => setViaRelayer(e.target.checked)}
                    />
                    <span>
                      Withdraw / send <b>via relayer</b>: the relayer submits
                      for you, so the tx doesn't come from your wallet (hides
                      your tx-origin) and costs you no gas. Uncheck to submit
                      yourself.
                    </span>
                  </label>
                  ...
                  {relayFallback && (
                    ...
                    >
                      ...
                      >
                        ⚠ <b>Gasless relay is unavailable right now.</b> You can
                        complete this {relayFallback.noun} yourself, but
                        submitting directly makes the transaction come{" "}
                        <b>from your own wallet</b>, revealing your address
                        on-chain. The relay exists precisely to hide it. Your
                        proof is already built; this only changes who submits
                        it.
                      </div>
                      <div
                        className="live-row"
                      ...
                      >
                        <button
                          className="btn btn-gold"
                          disabled={!!busy}
                          onClick={() => relayFallback.proceed()}
                        >
                          {busy
                            ? "submitting…"
                            : "Submit directly (reveals my address)"}
                        </button>
                        <button
                          className="btn"
                          disabled={!!busy}
                          onClick={() => setRelayFallback(null)}
                        >
                          Cancel
                        </button>
                      </div>
                  ...
                  {/* Shield (deposit) */}
                  <div className="pf-note" style={{ marginBottom: 6 }}>
                    Heads-up: your <b>first</b> shield downloads a one-time ~12
                    MB proving key, so it takes ~30s (quick after that). The
                    proof runs entirely in your browser, so nothing is sent
                    anywhere.
                  </div>
                  <div
                    className="live-row"
                  ...
                  >
                    <input
                      ...
                      placeholder={`amount to ${isSupply ? "supply" : "shield"} (${inputUnit})`}
                      inputMode="decimal"
                      value={shieldAmt}
                    ...
                    <button
                      className="btn btn-gold"
                      disabled={!!busy || (selPool.stock && impaired)}
                      onClick={shield}
                    >
                      {busy === "shield"
                        ? stage
                  ...
                  >
                    <div className="live-note">
                      Send privately to another user (in-pool, nothing is
                      unshielded):
                    </div>
                    <input
                      className="live-input"
                    ...
                    >
                      <input
                        className="live-input"
                        placeholder={`amount to send (${inputUnit})`}
                        inputMode="decimal"
                        value={xferAmt}
                    ...
                    <div className="pf-note">
                      The recipient must have unlocked + registered{" "}
                      <b>in this pool</b>. It appears in their shielded balance;
                      no amount leaves the pool, so it's fully private.
                    </div>
                  </div>
                  ...
                  >
                    ...
                    />
                    <div className="pf-note">
                      ⚠ Withdrawing to your own main wallet (or in an amount
                      that matches your deposit) can re-link you on a small
                      pool. Use a <b>fresh</b> address for stronger privacy.
                    </div>
                    <div
                      className="live-row"
                      style={{ gap: 10, flexWrap: "wrap" }}
                    >
                      <input
                        className="live-input"
                        placeholder={`amount to ${isSupply ? "withdraw" : "unshield"} (${inputUnit})`}
                        inputMode="decimal"
                        value={unshieldAmt}
                    ...
                    {isSupply && (
                      <div className="pf-note">
                        Amounts are in USDG; your position is held as
                        yield-bearing shares, so a withdrawal redeems the
                        equivalent shares and pays USDG including accrued yield
                        (≈, since the share price moves).
                      </div>
                    )}
                  </div>

                  <div className="pf-note" style={{ marginTop: 12 }}>
                    Notes are encrypted to your key and stored on-chain, so they
                    recover from any device by scanning. Nothing is tied to this
                    browser. Experimental — not yet live on mainnet.
                  </div>
                </div>
              )}
            </div>
              ...
              {!keys ? (
                  ...
                  >
                    ...
                    {registered === false ? (
                      ...
                      <button
                        ...
                        onClick={unlock}
                      >
                        {busy === "unlock" ? "unlocking…" : "Unlock & scan"}
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div className="live-card">
                  <div className="live-note" style={{ marginBottom: 8 }}>
                    Your reusable receiving handle. Share it, or just tell
                    senders your wallet address (it resolves to this):
                  </div>
                  <div
                    className="num"
                    style={{
                  ...
                  <div className="live-row" style={{ gap: 10 }}>
                    <button
                      className="pf-link"
                      onClick={() =>
                        navigator.clipboard?.writeText(keys.metaAddress)
                      }
                    >
                      copy meta-address
                    </button>
                    <button
                      ...
                      disabled={!!busy}
                      onClick={() =>
                        run("rescan", () => refreshInbox(keys), "✓ Rescanned.")
                      }
                    >
                      refresh inbox
                    </button>
            ...
            <div className="pf-block">
              ...
              <div className="live-card">
                <div className="live-note" style={{ marginBottom: 10 }}>
                  Pay anyone who has set up a private address. Enter their{" "}
                  <b>wallet address</b> (it resolves to their receiving handle)
                  or paste their meta-address.
                </div>
                <div
                ...
                >
                  <input
                    className="live-input"
                    placeholder="recipient wallet address (0x…) or meta-address"
                    value={to}
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={send}
                    >
                      {busy === "send" ? "sending…" : "Pay privately →"}
                    </button>
                  </div>
                  <button
                    className="pf-link"
            ...
            <div className="pf-block">
              ...
              {keys && inbox && inbox.length > 0 && (
                <div className="live-card" style={{ marginBottom: 12 }}>
                  ...
                  <div className="pf-note" style={{ marginTop: 8 }}>
                    ⚠ Sweeping spends gas from your <b>main wallet</b> and moves
                    the funds to the address above. Both write an on-chain link
                    between the one-time address and those wallets. For stronger
                    unlinkability, sweep to a <b>fresh</b> address;
                    relayer-funded gas (no main-wallet link) comes in a later
                    phase.
                  </div>
                </div>
              )}
              {!keys ? (
                <div className="pf-empty">
                  Unlock above to scan for payments only you can see.
                </div>
              ) : inbox === null ? (
                <div className="pf-empty">Scanning…</div>
              ) : inbox.length === 0 ? (
                <div className="pf-empty">
                  No private payments found yet. When someone pays your address,
                  it shows up here, visible only to you.
                </div>
              ) : (
                  ...
                  {inbox.map((h) => (
                    <div className="pf-seat" key={h.stealthAddress}>
                      <div className="pf-seat-top">
                        <span className="pf-seat-id num">
                          {short(h.stealthAddress)}
                        </span>
                        <a
                          className="pf-link"
                          href={`${NET.explorer}/address/${h.stealthAddress}`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          one-time address ↗
                      ...
                      {h.balances.length === 0 ? (
                        ...
                        h.balances.map((b) => (
                          <div className="pf-seat-row num" key={b.addr}>
                            <span>
                              <b>{fmt(b.amount, b.key === "USDG" ? 2 : 4)}</b>{" "}
                              {b.key}
                            </span>
                            <button
                              className="pf-link gold pf-inline-btn"
                            ...
                            >
                              {busy === "sweep" + h.stealthAddress + b.addr
                                ? "sweeping…"
                                : "sweep →"}
                            </button>
                          </div>
              ...
              {keys && (
                <div className="pf-note" style={{ marginTop: 10 }}>
                  Your stealth keys are derived from one signature and live in
                  this browser tab only, never stored or sent. Sign again
                  anytime to recover them on another device (standard EOA
                  wallets only).
                </div>
              )}
            </div>
            ...
            {msg && (
              <div className="live-msg" style={{ marginTop: 14 }}>
                {msg}
              </div>
            )}
            <div className="pf-note" style={{ marginTop: 18 }}>
              New here?{" "}
              <Link to="/portfolio" className="pf-link">
                Portfolio
              </Link>{" "}
              shows your public balances. Essey Private is an early primitive,
