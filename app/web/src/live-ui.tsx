...
function DonThumb({ id }: { id: bigint }) {
  ...
  return (
    <div className="amm-thumb">
      <div className="amm-ph" aria-hidden>
        <EMonogram size={40} />
        <span className="num">#{id.toString()}</span>
      </div>
      {!failed && (
        <img
          className="amm-img"
  ...
  return (
    <section className="band" id="trade" style={{ paddingTop: 0 }}>
      <div className="wrap">
        ...
        <div className="band-head amm-head">
          <div>
            <span className="eyebrow">The Exchange · live</span>
            <h2>Trade Dons: see them, click one, done</h2>
            <p>
              The live Don AMM, on-chain right now. Buy the next Don off the
              shelf at 8%, snipe an exact # at 12%, or sell one back. Every
              price is the reserve floor, read fresh. No wallet needed to browse
              the pool below.
            </p>
          </div>
        </div>
        <div className="live-card amm">
          ...
          <div className="ex-stats num amm-stats">
            <span>
              <b>{float_ !== null ? float_.toString() : "…"}</b> in the pool
            </span>
            <span className="ex-price">
              <b>{quote ? fmt(quote.price) : "…"}</b> $ESSEY <i>live floor</i>
            </span>
            <span>
              next at 8% <b>{quote ? fmt(quote.buyTotal) : "…"}</b> · snipe at
              12% <b>{quote ? fmt(quote.snipeTotal) : "…"}</b>
            </span>
            {bal && (
              <span>
                you own <b>{bal.dons.toString()}</b>
              </span>
            )}
          </div>
          ...
          {tab === "buy" && (
            <>
              ...
              <div className="amm-next">
                <div className="amm-next-copy">
                  ...
                  <span className="live-note">
                    Takes whichever Don is next off the shelf, the cheapest way
                    in at the <b>8%</b> fee. Don't care which #? Use this.
                  </span>
                </div>
                {!ready ? (
                  <ConnectButton />
                  ...
                  <Link className="btn btn-gold amm-next-btn" to="/faucet">
                    Get $ESSEY on the Faucet →
                  </Link>
                ) : (
                  <button
                    className="btn btn-gold amm-next-btn"
                    disabled={!!busy || float_ === 0n || !quote}
                    onClick={() =>
                      act(
                        "buy",
                        () =>
                          flows
                            ...
                            .then(({ id }) =>
                              setMsg(
                                `✓ Don #${id} is yours. Next: take him to the Game — the desk is waiting.`,
                              ),
                            ),
                        "✓ Don purchased",
                      )
                  ...
                  >
                    {busy === "buy"
                      ...
                      : float_ === 0n
                        ? "Pool is empty"
                        : quote
                          ? `Buy next · ${fmt(quote.buyTotal)} $ESSEY`
                          : "quoting…"}
                  </button>
                )}
              </div>
              ...
              {pick !== null && (
                <div className="amm-confirm">
                  ...
                  <div className="amm-confirm-body">
                    <div className="amm-confirm-h">
                      Snipe Don #{pick.toString()}{" "}
                      <span className="amm-fee-pill">12% premium</span>
                    </div>
                    <div className="amm-confirm-sub num">
                      {quote ? fmt(quote.price) : "…"} + 12% fee (
                      {quote ? fmt(quote.snipeFee) : "…"}) ={" "}
                      <b>{quote ? fmt(quote.snipeTotal) : "…"} $ESSEY</b>
                    </div>
                    <div className="live-note">
                      Picking an <b>exact</b> Don pays the 12% snipe fee,{" "}
                      {quote ? fmt(quote.snipeFee - quote.buyFee) : "…"} $ESSEY
              ...
              {poolIds === null ? (
                ...
                <div className="amm-empty live-note">
                  Every Don is held right now. Nothing on the shelf. Watch the{" "}
                  <Link to="/tape">Tape</Link> for one returning (holders can
                  sell back anytime).
                </div>
              ) : (
                <>
                  ...
                  <div className="amm-grid">
                    {poolIds.map((id) => (
                      ...
                      >
                        ...
                        <span className="amm-price num">
                          {quote ? fmt(quote.snipeTotal) : "…"}
                        </span>
                      </button>
                    ))}
                  </div>
              ...
              <div className="live-note ex-help">
                🛡 Every Don carries a hard <b>$ESSEY floor</b>: redeem it for
                its share of the reserve anytime (see{" "}
                <Link to="/portfolio">Portfolio</Link>). The floor only ever
                rises. Totals carry ~1% slippage headroom; you're only charged
                the on-chain quote. Need funds?{" "}
          ...
          {tab === "sell" &&
            (!ready ? (
              ...
              <div className="amm-empty live-note">
                You don't own a Don yet.{" "}
                <button className="linklike" onClick={() => setTab("buy")}>
                  Buy one from the pool →
                </button>
              </div>
            ) : (
              <>
                {sellPick !== null && sellDon && (
                  <div className="amm-confirm">
                    ...
                    <div className="amm-confirm-body">
                      <div className="amm-confirm-h">
                        Sell Don #{sellPick.toString()}{" "}
                        <span className="amm-fee-pill">−8% fee</span>
                      </div>
                      <div className="amm-confirm-sub num">
                        receive <b>{quote ? fmt(quote.sellNet) : "…"} $ESSEY</b>{" "}
                        <i>({quote ? fmt(quote.price) : "…"} floor − 8%)</i>
                      </div>
                      <div className="live-note">
                        Returns the Don to the pool at the live floor minus the
                        8% fee. Its Vault contents travel with it, so{" "}
                        <b>claim and empty the Vault first</b> (see{" "}
                    ...
                    <div className="amm-confirm-act">
                      <button
                        ...
                        onClick={() =>
                          act(
                            "sell",
                            () =>
                              flows.sellDon(a!, sellPick).then(() => {
                                setMsg(
                                  `✓ sold Don #${sellPick} back for ~${quote ? fmt(quote.sellNet) : ""} $ESSEY`,
                                );
                                setSellPick(null);
                              }),
                            "✓ sold",
                      ...
                      >
                        {busy === "sell"
                          ? "selling…"
                          : `Confirm sell · get ~${quote ? fmt(quote.sellNet) : "…"}`}
                      </button>
                      <button
                        className="btn btn-ghost"
                        onClick={() => setSellPick(null)}
                      >
                        Cancel
                      </button>
                    </div>
                ...
                <div className="amm-grid-h live-note">
                  Tap one of your Dons to sell it back at the live floor −8%.
                </div>
                <div className="amm-grid">
                  {owned.map((d) => (
                    <button
                      ...
                      className={
                        "amm-card" +
                        (sellPick === d.id ? " on" : "") +
                        (d.liened ? " amm-card-off" : "")
                      }
                      disabled={d.liened}
                      aria-pressed={sellPick === d.id}
                      title={
                        d.liened
                          ? "Loan collateral: repay the loan to unlock"
                          : undefined
                      }
                      onClick={() => {
                        if (!d.liened) {
                    ...
                    >
                      ...
                      {d.liened ? (
                        <span className="amm-badge amm-badge-lien">
                          📜 liened
                        </span>
                      ) : d.locked ? (
                ...
                <div className="live-note ex-help">
                  A <b>liened</b> Don is loan collateral. Repay the loan
                  (Portfolio) to unlock it before selling. A <b>staked</b> Don
                  sells fine; its Bell tier just clears on transfer.
                </div>
              </>
            ))}
          ...
          {busy && (
            <div className="live-note ex-help">
              This can take a couple of wallet confirmations. Your wallet will
...
const MIX_TOKENS: { key: string; addr: Address }[] = [
  { key: "Bundle", addr: BUNDLE },
  { key: "AAPL", addr: ADDR.aapl },
  { key: "NVDA", addr: ADDR.nvda },
];
...
export function LiveBell() {
  ...
  useEffect(() => {
    loadPot();
    const t = setInterval(loadPot, 15_000);
    return () => clearInterval(t);
  }, [loadPot]);
  useEffect(() => {
    loadDons();
  ...
  const applyMix = () => {
    const picked = MIX_TOKENS.map((t) => ({
      token: t.addr,
      key: t.key,
      pct: Number(mixPct[t.key] || "0"),
    })).filter((t) => t.pct > 0);
    return act(
      "pref",
      () =>
        flows.setPayoutMix(
          a!,
          sel!,
          picked.map((t) => ({ token: t.token, bps: t.pct * 100 })),
        ),
    ...
    <section className="band" id="bell">
      <div className="wrap">
        ...
        {!ready ? (
          ...
          <div className="live-card">
            <div className="live-note">
              You need a Don first.{" "}
              <Link to="/market">Buy one on the Exchange →</Link> then come back
              to stake it and start earning.
            </div>
          </div>
        ) : (
          <>
            {dons.length > 1 && (
              <div className="bell-seat-pick">
                ...
                {dons.map((id) => (
                  <button
                    key={id.toString()}
                    className={"seat-pill num" + (sel === id ? " on" : "")}
                    onClick={() => setSel(id)}
                  >
                    #{id.toString()}
                  </button>
            ...
            {state && (
              <>
                ...
                <div className="live-card bell-step">
                  <div className="bell-step-h">
                    <span className="bell-step-n">1</span> Stake your Don
                    <span className="bell-step-cur">
                      {state.tier === 0
                        ? "not staked yet"
                        : `staked · ${TIERS[state.tier - 1]?.name ?? `Tier ${state.tier}`}`}
                    </span>
                  </div>
                  <div className="live-note">
                    Stake $ESSEY on Don #{sel?.toString()} to start earning from
                    every payout. Higher tier = a bigger share. Half of every
                    stake burns, half funds the treasury. Pick a level:
                  </div>
                    ...
                    {TIERS.map((t) => {
                      ...
                      return (
                        <button
                          key={t.tier}
                          className={
                            "tier-btn" + (t.tier === owned ? " on" : "")
                          }
                          disabled={disabled}
                          onClick={() =>
                            act(
                              "tier",
                              () => flows.setTier(a!, sel!, t.tier),
                              `✓ Don #${sel} is now ${t.name}`,
                            )
                          }
                        >
                          <b>{t.name}</b>
                          <i className="num">
                            {(t.weight / 100).toFixed(2)}× share · {fmt(t.fee)}{" "}
                            $ESSEY
                          </i>
                  ...
                  <div className="live-note">
                    {state.locked ? (
                      <>
                        🔒 This Don's art is <b>locked</b>. Its look is
                        permanent.
                      </>
                    ) : (
                      <>
                        Heads up: activating a Don{" "}
                        <b>locks its art permanently</b>. Reroll or customize it
                        on the <Link to="/builder">Builder</Link> first if you
                    ...
                    {state.liened && (
                      <>
                        {" "}
                        · 📜 <b>collateralized</b> on a loan. Still staked,
                        still earning; it just can't move until the debt clears.
                      </>
                    )}
                  </div>
                </div>
                ...
                <div className="live-card bell-step">
                  <div className="bell-step-h">
                    <span className="bell-step-n">2</span> Ring the Bell
                    <span className="bell-step-cur num">
                      pot: {pot !== null ? fmt(pot, 2) : "…"} USDG
                    </span>
                  </div>
                  <div className="live-note">
                    When the pot's worth it, <b>anyone</b> can ring it. Ringing
                    costs only gas and pays the ringer nothing; the whole pot
                    splits across all staked Dons (by tier). The pot grows from
                    protocol fees: trades and loan interest.
                  </div>
                  <button
                    className="btn btn-gold"
                    disabled={!!busy || (pot ?? 0n) === 0n}
                    onClick={() =>
                      act(
                        "ring",
                        () => flows.ringBell(a!),
                        "✓ rung. The pot split across staked Dons",
                      )
                    }
                  >
                    {busy === "ring"
                      ? "ringing…"
                      : (pot ?? 0n) === 0n
                        ? "Pot is empty, grow it first"
                        : "Ring the Bell"}
                  </button>
                  {(pot ?? 0n) === 0n && (
                    <div className="live-note">
                      <Link to="/market">→ Trade on the Exchange</Link> to grow
                      the pot, then come back to ring.
                    </div>
                  )}
                </div>
                ...
                <div className="live-card bell-step">
                  <div className="bell-step-h">
                    <span className="bell-step-n">3</span> Claim your payout
                    <span className="bell-step-cur num">
                      yours: {fmt(state.pending, 4)}{" "}
                      {CONVERTER_LIVE ? "→ stock" : "USDG"}
                    </span>
                  </div>
                  {CONVERTER_LIVE && (
                  ...
                  {CONVERTER_LIVE && mixOpen && (
                    <div className="payout-pref">
                      <span className="pp-label">
                        Split across up to 3 (must total 100%)
                      </span>
                      {MIX_TOKENS.map((t) => (
                        <label
                          key={t.key}
                          className="live-note num"
                          style={{
                            display: "inline-flex",
                            alignItems: "center",
                            gap: 4,
                        ...
                        >
                          {t.key}
                          <input
                            className="num ex-input"
                            style={{ width: 56 }}
                            type="text"
                            inputMode="numeric"
                            placeholder="0"
                            value={mixPct[t.key] ?? ""}
                            aria-label={`${t.key} percent`}
                            onChange={(e) =>
                              setMixPct((m) => ({
                                ...m,
                                [t.key]: e.target.value.replace(/\D/g, ""),
                              }))
                            }
                          />
                          %
                  ...
                  <div className="live-row">
                    <button
                      className="btn btn-gold"
                      disabled={!!busy || state.pending === 0n}
                      onClick={() =>
                        act(
                          "claim",
                          () => flows.claimPayout(a!, sel!),
                          `✓ Payout claimed into Don #${sel}'s Vault`,
                        )
                      }
                    >
                      {busy === "claim"
                        ...
                        : state.pending === 0n
                          ...
                          : CONVERTER_LIVE
                            ? `Claim ${fmt(state.pending, 4)} as ${prefKey(pref)}`
                            : `Claim ${fmt(state.pending, 4)} USDG`}
                    </button>
                    <a
                      className="btn btn-ghost"
                      href={`${NET.explorer}/address/${state.vault}`}
                      target="_blank"
                      rel="noreferrer"
                    >
                      view your Vault ↗
                  ...
                  <div className="live-note num">
                    Your Vault holds{" "}
                    {vaultBal !== null ? fmt(vaultBal, 2) : "…"} USDG
                    {vaultStock &&
                      (vaultStock.aapl > 0n || vaultStock.nvda > 0n) && (
                        <>
                          {" "}
                          ·{" "}
                          <b>
                            {vaultStock.aapl > 0n &&
                              `${fmt(vaultStock.aapl, 2)} AAPL`}
                            {vaultStock.aapl > 0n && vaultStock.nvda > 0n
                              ? " · "
                              : ""}
                            {vaultStock.nvda > 0n &&
                              `${fmt(vaultStock.nvda, 2)} NVDA`}
                          </b>
                        </>
                      )}{" "}
                    and it travels with the Don if you ever sell it.
                  </div>
                </div>
              </>
            )}
