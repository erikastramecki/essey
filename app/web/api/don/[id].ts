...
/// The Don's stat sheet, decoded from the same preimage the art is keyed by. Display-only: the game
/// reads attested sheets, not this. Never throws — the desk must keep listing Dons when the registry
/// is unreachable, and 11 Dons have permanently lost preimages.
async function sheetFrom(preimage: string) {
  try {
    const s = await chainClient().readContract({
      address: AFFINITY_ADDR,
      abi: affinityAbi,
      functionName: "previewSheet",
      args: [toHex(preimage)],
    });
    return { ...s, archetype: ARCHETYPES[s.archetype] ?? "Unspecialized" };
  } catch {
    return null;
  }
}
...
export default async function handler(req: NodeReq, res: NodeRes) {
  ...
  const redis = store();
  const pre = redis
    ? await redis.get<Preimage>(preKey(combo)).catch(() => null)
    : null;
  ...
  if (pre) {
    try {
      ...
      if (comboHash(r.key).toLowerCase() !== combo.toLowerCase())
        throw new Error("registry/chain mismatch");
      return json(
        res,
        200,
        {
        ...
        {
          "cache-control":
            CACHE_SHORT /* lien state is mutable even when art is locked - audit R5 LOW */,
        },
      );
    } catch {
