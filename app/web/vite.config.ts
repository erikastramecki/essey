          ...
          if (req.method === "POST") {
            ...
            const data = JSON.parse(
              fs.readFileSync(
                fileURLToPath(
                  new URL(
                    `./public/builder/data_${gender}.json`,
                    import.meta.url,
                  ),
                ),
                "utf8",
            ...
            if (store[r.key])
              return send(res, 200, {
                ...
                taken: true,
                key: r.key,
                by: store[r.key].wallet,
              });
            store[r.key] = { gender, wallet: wallet || "anon", ts: Date.now() };
            write(store);
...
export default defineConfig({
  // The Holder Hub stays preview-only until its distributor deploys (founder standing rule). This flag
  // is false ONLY on a production Vercel build, so a fresh prod build drops the /holder route; the
  // hostname check in App.tsx is the second guard, covering an alias-promote of a preview build.
  define: {
    __HOLDER_BUILD__: JSON.stringify(process.env.VERCEL_ENV !== "production"),
  },
  ...
  plugins: [
    nodePolyfills({
      include: ["buffer", "events", "stream", "util", "process", "assert"],
  ...
  resolve: {
    alias: {
      "@essey/sui-sdk": fileURLToPath(
        new URL("../sui-sdk/src/index.ts", import.meta.url),
      ),
    },
