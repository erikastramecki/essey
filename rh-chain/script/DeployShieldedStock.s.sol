...
// Essey Private — deploy a SHIELDED STOCK pool for one tokenized-stock token (AAPL or NVDA). Reuses the
// depth-20 verifier + Poseidon hasher. Notes are raw stock amounts; a pro-rata haircut survives issuer
// adminBurn of the pool's backing. Run once per stock token:
//
//   STOCK=$AAPL PK=$TESTNET_DEPLOYER_PK \
//   forge script script/DeployShieldedStock.s.sol --rpc-url rh_testnet --broadcast --private-key $PK \
...
contract DeployShieldedStock is Script {
  ...
  function run() external {
    ...
    EsseyShieldedStock pool = new EsseyShieldedStock(
      IPoolVerifier(address(verifier)), LEVELS, hasher, IERC20(stock), IEsseyPoolGate(address(gate)), msg.sender, type(uint256).max
    );
    vm.stopBroadcast();
    console.log("StockToken   ", stock);
    console.log("StockHasher  ", hasher);
    console.log("StockVerifier", address(verifier));
    console.log("StockGate    ", address(gate));
    console.log("ShieldedStock", address(pool));
  }
