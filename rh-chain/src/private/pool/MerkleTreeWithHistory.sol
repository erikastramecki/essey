...
/// Poseidon(2) hasher — an external contract (generated from circomlib's poseidon_gencontract) whose
/// hashing MUST match the circuit's merkleProof Poseidon(2), or on-chain roots won't match proofs.
interface IHasher {
  function poseidon(bytes32[2] calldata inputs) external pure returns (bytes32);
}

/// Incremental Merkle tree with a rolling root history, ported from Tornado Nova (MIT) to Solidity 0.8.28
/// and de-upgradeable-ified: the empty-subtree fill happens once in the constructor instead of an
/// initializer. Leaves are inserted in pairs (one shielded transaction emits two output commitments).
///
/// The `zeros()` table is the empty-subtree roots (z[0]=ZERO_VALUE, z[i]=Poseidon(z[i-1],z[i-1])) computed
/// under MODERN circomlib Poseidon — the SAME hash the ported circuit and the deployed IHasher use — so a
/// proof's merkle root matches this tree's root. (Nova's original table used old-tornado Poseidon; it is
/// NOT interchangeable — mixing them makes isKnownRoot reject every proof.)
contract MerkleTreeWithHistory {
  ...
  /// Poseidon(_left, _right) via the external hasher.
  ...
  /// Insert a pair of leaves and push a new root into the history ring.
  ...
  /// Is `_root` in the recent-root ring? (Proofs may be built against a slightly stale but still-valid root.)
  ...
  /// Precomputed empty-subtree roots under MODERN circomlib Poseidon (z[0]=ZERO_VALUE, z[i]=P(z[i-1],z[i-1])).
  /// Regenerated to match the ported circuit + IHasher; verified zeros(5) == the circuit's empty-tree root.
