...
///
/// Phase 0 is the Scrip-only playable core (GAME-CONCEPT §4.4 "Skirmish v2"): mission board +
...
/// The generation registry (contracts-architecture §2.16/§5): fixed role list, timelocked
...
/// The fixed role vocabulary. IMMUTABLE at deploy per the ruled slate: A14 requires the reserved
/// OUTFIT_MODULE slot to exist NOW (the list can never grow), even though no Outfit contract ships
...
library GameRoles {
    ...
    bytes32 internal constant OUTFIT_MODULE = "OUTFIT_MODULE"; // reserved (Phase 1.5, per A14)
    bytes32 internal constant KEEPER = "KEEPER";
}

/// Season-scoped, non-transferable points (contracts-architecture §2.13). Mint/burn/move only by
...
interface IMissionBoard {
    ...
    /// The garrison commit frozen at depart (gas law §1.3): zero while home or if the player
